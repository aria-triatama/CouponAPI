...
func CheckStock(mdb DBstore, data models.Claims) bool {
	var result models.Coupons

	coupon := CouponsCollection(mdb)

	err := coupon.FindOne(*mdb.GetCtx(), bson.M{"name": data.CouponName}).Decode(&result)
	if err != nil {
		return false
	}
...
func UpdateStock(mdb DBstore, data models.Claims) error {
	coupon := CouponsCollection(mdb)

	_, err := coupon.UpdateOne(*mdb.GetCtx(), bson.M{"name": data.CouponName}, bson.M{"$inc": bson.M{"remaining_amount": -1}})
	if err != nil {
		return err
	}
